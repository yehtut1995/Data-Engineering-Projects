...
# look to the `args` object with the parameters required to run the Job
...
# Create a new item named `"session_item"` by applying the `explode()` method to the `session_items` field
sessions_df = sessions_df.withColumn("session_item", explode("session_items"))

# Then, apply the `select` method of the `sessions_df` Spark Dataframe with the `col()` method to select the required columns
